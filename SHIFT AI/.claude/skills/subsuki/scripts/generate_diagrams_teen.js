/**
 * generate_diagrams_teen.js — サブスキちゃん専用：中高生向け図解画像を生成
 *
 * slide-generator-main/scripts/generate_diagrams.js のコピー改変版。
 * プロンプト内部のビジネス前提（ビジネスパーソン／オフィス／本業）を
 * 学校生活前提（学生／制服・カジュアル／学校空間）に差し替えている。
 *
 * それ以外の仕様（分類器／パターン割当／リトライ／サイズチェック）は
 * 元版と同一。
 *
 * Usage:
 *   node generate_diagrams_teen.js <slides.json> <output_dir> [--env <.env_path>]
 */
...
function buildDiagramPrompt(title, body, script, assignedPattern) {
  ...
  return `2400x840pxの学習インフォグラフィック画像を生成してください。
...
# 対象ユーザー
この図解は中高生向けオンライン学習教材に使用される。
- 視認性を最優先: 文字は大きく明瞭に、コントラストを十分に確保する
- 中高生が見て「自分ごと」と感じられるテイスト。制服・カジュアル服の学生、教室・机・ノート・プリント・図書室・リビング学習・スマホ学習など、**学校生活を中心とした世界観**で構成する。スーツのビジネスパーソン・オフィス・会議室・商談シーンは**絶対に描かない**
- 親しみやすさと知的さの両立: 子供っぽいキャラクター化はせず、中高生（13〜18歳）のリアルな等身感・生き生きした雰囲気を保つ。フラット・アイソメトリックイラストを活用しつつ、教育的でクリーンな印象にする
...
# 配色（最大4色 + 白背景）
- 背景: 白固定 (#ffffff)
- 枠線・ボーダー・ヘッダー帯: ダークグレー (#333333) をベースカラーとして使用
- テキスト色: ダークグレー (#333333) のみ。マゼンタ (#A51E6D) 以外の色（紫・オレンジ・緑・赤・青など）でテキストを強調することは絶対禁止
- 強調色: マゼンタ (#A51E6D) のみ。図解内でテキストを強調する唯一の色。最も伝えたいキーワード（1〜2語）にのみ使用。多用しない
- ボックス背景色:
  - 基本は白 (#ffffff)。すべてのボックスのデフォルト背景は白
  - 強調したい要素も含め、すべてのボックス背景は白 (#ffffff) を使う
- イラスト配色: インフォグラフィックを意識し、マゼンタ (#A51E6D) の強調色が際立つよう、イラスト部分は落ち着いたニュートラルカラー（ネイビー #2C3E50・スレートブルー #5D6D7E・ティール #1ABC9C・ウォームグレー #95A5A6・ベージュ #F5F0EB・淡いブルーグリーン #D5E8D4）を中心に構成する。マゼンタと競合する鮮やかな色（紫・オレンジ・赤・ビビッドな緑）はイラスト内でも避ける
...
- 清潔感・余白・洗練された親しみやすさを最優先（中高生が見て「勉強したくなる」テイスト。教室・図書室・自宅の机など学校生活を想起させる小物を適度に配置）
- デュアルコーディング: 図形と短いテキストラベルを組み合わせる

# ビジュアル技法（インフォグラフィック品質向上）
- フラット・アイソメトリックイラスト: 人物は1図解に最大1人。人物は中高生（13〜18歳）として描き、制服（学生服・セーラー服・ブレザー）またはカジュアル服（パーカー・Tシャツ・ジーンズ）を着用。頭身は6〜7頭身のリアルな体型比率で、生き生きとした学生の雰囲気を保つ。デフォルメしすぎない（2〜3頭身の子供っぽいキャラクターは禁止）。フラット・アイソメトリックスタイル（等角投影＋ベタ塗り）で描く。人物なしでも可。**スーツ・ネクタイ・オフィスカジュアル・ビジネスパーソンは絶対に描かない**
...
- **色名ワード（「マゼンタ」「magenta」「ネイビー」「グレー」「ダークグレー」「ティール」等、日本語・英語問わず色を表す単語）を図解内のテキストラベルとして絶対に描画しない。これらは色指定用のメタ指示であり、図解内のラベル・見出し・注釈に文字として出してはならない**
...
- イラスト内にマゼンタと競合する鮮やかな色（ビビッドな紫・オレンジ・赤・緑など）を使用すること（イラストはネイビー #2C3E50・スレートブルー #5D6D7E・ティール #1ABC9C・グレー系の落ち着いた色で構成）
- 写真・実写風の表現
- 「図解に含めるテキスト」セクション以外の文字列を図解内に描画しない
- テーマ・タイトル・メタ情報・指示文・禁止事項の文章を図解内に描画しない
- 「ビジネスインフォグラフィック」「Business Infographic」「学習インフォグラフィック」等のメタ的なタイトル・ラベルの表示（図解の種類や形式名を図解内に日本語でも英語でも書かない。独自の英語タイトルも禁止）
- スーツ・ネクタイ・オフィスカジュアル・ビジネスパーソン・会議室・商談・オフィスデスクの描写
- 「本業」「業務」「ワークフロー」「ミーティング」「顧客」「クライアント」等のビジネス語彙を図解内テキストとして表示すること
- 要素の詰め込み（5個以上の要素は禁止）
