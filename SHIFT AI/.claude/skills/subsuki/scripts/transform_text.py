...
def format_body_lines(text: str, slide_no=None, diffs=None):
    """Body を extractDiagramContent が headings として拾える形に整形する。

    ルール（D-4案・2026-04-23 更新 / master 指示）:
      - 句点「。」で文を分割し、各文を改行区切りで連結する
      - 各文の末尾の句点は削除する
      - 既に改行区切りの文章は、各行ごとに同じ処理を適用する
      - 空行はスキップ

    目的:
      slide-generator の extractDiagramContent() は行頭「・/-/数字.」または
      50文字未満＋句点なしの行だけを抽出する。Body が句点で終わる長文1行だと
      bullets にも headings にも入らず、Gemini に「テキストなし」と伝わる。
      句点で分割して各文を独立させ、末尾句点を削除することで headings として
      抽出可能な形に整える。行頭「・」は付けない（マスター指示・ノイズ回避）。
    """
    if not text:
        return text
    lines = text.split("\n")
    ...
    for original in lines:
        ...
        if parts != [line.rstrip("。")]:
            changed = True
    # 元テキストと比較して変更があったかチェック
    new_text = "\n".join(formatted)
    if new_text != text and diffs is not None:
        diffs.append({
            "slide": slide_no,
            "field": "BodyFormat",
            "before": "（従前の文章形式）",
            "after": "（句点で改行分割＋句点削除）",
            "context": "D-4案 句点分割ルール適用",
        })
    return new_text
...
def transform_slide_design(src: Path, dst: Path, mappings, diffs):
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        if body_cell.value:
            ...
            # Step 2: D-4案 Body 整形（句点で改行分割＋句点削除 / master 2026-04-23 指示）
