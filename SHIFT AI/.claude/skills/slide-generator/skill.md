...
- 人物は1図解に最大1人、5-6頭身のスタイリッシュなフラットイラスト
...
**図解カラースキーム（教育コンテンツパレット + 白背景）:**
- アクセント色（最も目立つ色）: マゼンタ `#A51E6D`（キーワード強調、強調ボーダー）
- 文字色: `#333333`
- イラスト色（すべてマゼンタより控えめ）: スモーキーブルー `#6E9ECF` / ソフトオレンジ `#E8A87C` / セージグリーン `#7DAF7D` / ウォームグレー `#9E9E9E`
- ボックス背景: すべて白 `#ffffff`（色付き背景は使わない）
- 全体背景: 必ず `#ffffff`（白）— 暗い背景は禁止
...
**図解仕様:**
- フラットインフォグラフィックスタイル（ベタ塗り + 微細シャドウ）
- 人物は最大1人、5-6頭身のスタイリッシュなフラットイラスト
- イラスト色はスモーキーブルー・ソフトオレンジ・セージグリーン等の控えめなトーン（マゼンタを邪魔しない）
- PNG 1200x420px、白背景必須 (#ffffff)
- 枠線: `#333333`（強調ボーダーのみ `#A51E6D`）、文字色: `#333333`
- キーワード強調: 最重要ワードをマゼンタ (`#A51E6D`) で表示（図解内で最も目立つ色）
- ボックス背景: すべて白（色付き背景は使わない、枠線でグルーピング）
- 禁止: パターン名・英語タイトル・メタラベルの図解内表示
