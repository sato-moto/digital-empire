...
2つの AI モデル（Claude / Gemini）を並列に使い、カリキュラム設計 → アウトライン制作 → 品質レビュー → スライド＆台本生成までを自動化します。

## ワークフロー

```
Phase 1  カリキュラム設計
  ├─ カリキュラムリサーチ（2モデル並列 → 統合）
  ├─ カリキュラム構成提案 + ユーザーと壁打ち
  └─ SET選択
         │
...
- **2モデル並列リサーチ** — Claude・Gemini が同じテーマで独立にリサーチを生成し、最良の案をマージ
...
└── scripts/                    API呼び出しスクリプト
    ├── call_parallel.py          Gemini 並列呼び出し（レガシー: 旧OpenAI並列対応）
