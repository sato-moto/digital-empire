...
教養/実務パイプラインの Step 3 と同じ2モデル並列パイプラインを実行する。
...
3. **並行して** api-caller が Gemini を API 呼び出し → `outline_gemini.md`
4. 相互批評（Step 5-A相当）→ `critique_claude.md`, `critique_gemini.md`
5. セルフレビュー 3段階（Step 5-B相当）→ `self_review_claude.md`, `self_review_gemini.md`
