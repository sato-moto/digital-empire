...
**スキップ判定:** `.venv` が親ディレクトリに存在し、`python -c "import openai; import google.genai"` が成功すればスキップ可能。ただし初回は必ず実行する。
...
**4b: OPENAI_API_KEY の確認**

値が `your-openai-api-key` のままか空の場合、ユーザーに入力を求める:

「OpenAI の API キーを入力してください。
3モデル並列生成（リサーチ・アウトライン）で使用します。
...
```bash
# Python 依存
cd "$(dirname "{skill_root}")" && .venv/Scripts/python -c "import openai; import google.genai; import openpyxl; import PIL; print('OK')" 2>&1 || .venv/bin/python -c "import openai; import google.genai; import openpyxl; import PIL; print('OK')" 2>&1
...
o = bool(os.getenv('OPENAI_API_KEY','')) and os.getenv('OPENAI_API_KEY') != 'your-openai-api-key'
print(f'GOOGLE_API_KEY: {\"設定済み\" if g else \"未設定\"}')
print(f'OPENAI_API_KEY: {\"設定済み\" if o else \"未設定\"}')
" 2>&1
```
