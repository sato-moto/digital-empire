#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""OpenAI API 呼び出しスクリプト
Usage:
    python call_openai.py --prompt <prompt_file> --output <output_file>
    python call_openai.py --prompt <prompt_file> --output <output_file> --merge <f1> <f2> ...
"""
import argparse
import os
import sys
...
def main():
    ...
    parser.add_argument("--model", default="openai/gpt-4o", help="Model name (OpenRouter format: openai/gpt-4o)")
    ...
    if not api_key:
        print("[ERROR] OPENAI_API_KEY が設定されていません。.env ファイルにAPIキーを設定してください（.env.example を参考）", file=sys.stderr)
        sys.exit(1)
