...
SET単位（①②選択時）:
  ...
  → Step 4〜6: 教養/実務で分岐
    ├─ 教養 → アウトライン生成（3モデル並列→批評→マージ→レビュー）→ outline-final.md
    └─ 実務 → 収録指示書生成 → practice-{N}.md + practice-{N}.csv
...
※ 実務講座は収録指示書が最終成果物のため Phase 3 は対象外
...
| Step | 内容 | 実行者 | 成果物 |
|---|---|---|---|
| Step 0 | カリキュラムリサーチ（3モデル並列→統合→ファクトチェック→セルフチェック） | メインClaude + api-caller | `research-summary.md` |
...
| Step | 内容 | 成果物 |
|---|---|---|
| Step 3 | 追加リサーチ（3モデル並列→統合） | `detailed-research.md` |
| Step 4 | アウトライン生成（3モデル並列） | `outline_claude/gemini/openai.md` |
...
|---|---|---|
| Step 3 | 追加リサーチ（3モデル並列→統合） | `detailed-research.md` |
| Step 4 | 収録指示書生成（3モデル並列、`practice_video_guideline.md` 準拠） | `practice_claude/gemini/openai.md` + `.csv` |
| Step 5 | 相互批評→セルフレビュー→マージ+構造lint+収録指示書チェック(P1-P8) | `practice-{N}.md` + `.csv` |
| Step 6 | 51項目レビュー + セット整合(#52-57) + 収録指示書チェック(P1-P8) | `practice-{N}-final.md` + `practice-{N}-final.csv` |

※ 実務の最終成果物は収録指示書（md + csv）。Phase 3/4 には進まない。
...
| Step 3 リサーチ | 下記6つを全て参照（教養+実務+指示書を一括でリサーチ） |

SET単位 Step 3 で読み込むGL一覧:
...
※ 実務の出力は `practice-{N}.md` + `practice-{N}.csv`（収録指示書）。Phase 3/4 には進まない。
