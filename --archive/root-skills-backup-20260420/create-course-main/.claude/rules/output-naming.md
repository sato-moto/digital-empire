...
**CSVファイルを変更したときは、対応するExcelファイル（.xlsx）も必ず同時に再生成すること。**
逆も同様（Excelを変更したらCSVも更新）。片方だけ更新して整合性が崩れることを防ぐ。

対象ペア:
- `slide-design.csv` ↔ `slide-design.xlsx`
- `script.csv` ↔ `script.xlsx`
- `practice-{N}.csv` ↔ `practice-{N}.md`（収録指示書）

## PPTX/PDF 同期ルール（必須）

**PPTXを手動編集した場合、対応するPDFも必ず再生成すること。**
build_slides.js のPDF生成はJSON→PPTXの新規ビルド時のみ有効。手動編集後のPDF反映にはPowerPointからのエクスポートが必要。
