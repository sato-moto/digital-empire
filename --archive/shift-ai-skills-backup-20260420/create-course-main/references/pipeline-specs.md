...
3. **3モデル並列生成** → メインClaude（Opus 4.6）が Claude版生成 ←並行→ api-caller が Gemini + OpenAI API 呼び出し
4. **各版ファクトチェック** → 3版それぞれに WebSearch で裏取り（統合前に品質担保）
5. **各版セルフチェック** → 3版それぞれに14項目チェック + 改善（統合前に品質担保）
6. **3版統合** → メインClaude がMECE統合（統合判断基準9項目）
...
2. **追加リサーチ** → 確定タイトル+ゴールで3モデル並列 → ★ユーザー確認
3. **章ロール設計** → メインClaude単独（outline_v1.md ③準拠）→ ★ユーザー確認+ブラッシュアップ
4. **章アウトライン設計** → 3モデル並列+フルパイプライン（outline_v1.md ④準拠）→ ★ユーザー確認+ブラッシュアップ
...
API呼び出しスクリプト:
- `{skill_root}/scripts/call_parallel.py` — Gemini + OpenAI 並列呼び出し
- `{skill_root}/scripts/call_openai.py` — OpenAI 単独呼び出し
- `{skill_root}/scripts/call_gemini.py` — Gemini 単独呼び出し
